...
*	Returns path array.
*/
uint8_t *setDrop(uint8_t position)
{
	...
	else
	{
		...
		for(uint8_t i = 7; i >= 1; i--)
		{
			...
			obstacle[i-1] = obstagral;
			
			writeMax(i, obstacle[i-1]);
			_delay_ms(150);
		}
	}
	return obstacle;
